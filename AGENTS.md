...
## Layout
- `config.py` — env-var settings (see `.env.example`) · `ircconn.py` — IRC client · `signaling.py` —
  WebRTC-over-IRC codec
- `publisher.py` — aiortc peer · `player.py` — queue + yt-dlp search/resolve · `commands.py` — chat
  commands · `fallback.py`/`jellyfin.py` — idle Jellyfin show · `api.py` — REST

Infra (k8s manifests) lives in the private homelab repo, not here; CI only builds and pushes the image.
